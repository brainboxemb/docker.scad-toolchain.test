...
SCAD runtime image family: OpenSCAD-focused, drawing and full/PythonSCAD.
...
representative PNG/STL generation, Git operations, docs tooling, PNG watermark
post-processing and the drawing profile's OpenSCAD -> scripted SVG -> Inkscape
PNG/PDF publication path.
...
## Drawing publication test

The drawing profile must be qualified as real end-to-end consumer paths, not
by checking only that commands exist.

For publication, the suite owns a small Python/SVG composition script. It
consumes SVG geometry generated by OpenSCAD, adds sheet/annotation content, and
requires Inkscape CLI to produce valid PNG and PDF outputs.

For CAD-style projection, the suite must consume a real OpenSCAD-generated STL,
convert it through FreeCAD Mesh -> Part solid -> refine/removeSplitter, and
require TechDraw HLR to emit meaningful SVG geometry. The OpenSCAD and full
profiles must not silently acquire Inkscape or FreeCAD.
...
Successful runs publish generated evidence for all three runtime profiles to Pages:
