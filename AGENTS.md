...
The drawing profile must be qualified as a real end-to-end consumer path, not
by checking only that `inkscape` exists.

The suite owns a small Python/SVG composition script. It consumes SVG geometry
generated by OpenSCAD, adds sheet/annotation content, and requires Inkscape CLI
to produce valid PNG and PDF outputs. The OpenSCAD and full profiles must not
silently acquire Inkscape.
